...
export default function ShortlistPage() {
  ...
  const ranked = [...apps]
    .filter((a) => a.eligibility?.status !== "notEligible")
    ...
    <div>
      ...
      <div className="mt-6 flex flex-col gap-3">
        {ranked.map((a, i) => (
          <div
            key={a.id}
            className="flex flex-wrap items-center justify-between gap-4 rounded-xl2 border border-navy/10 bg-white p-5 shadow-card"
          >
            <div className="flex items-center gap-4">
              <span className="w-6 text-center font-display text-lg font-semibold text-navy">
                {i + 1}
              </span>
              <div>
                <Link
                ...
                <p className="text-xs text-slate">
                  {a.recommendation.summary.slice(0, 90)}…
                </p>
              </div>
            </div>
